...
graph TD
    ...
    GitHub -->|2. Trigger Pipeline| Jenkins
    
    Jenkins -->|3. Terraform Provisioning| VPC
    Jenkins -->|4. Ansible SSH ProxyCommand| Bastion
    
    Bastion -->|SSH Forwarding| App
    User -.->|5. Local Port Forwarding 3000| Bastion
    
    App -->|Read/Write| RDS
    App -->|Cache| Redis
    
    App -->|Internet Access| NAT
    NAT --> IGW
    Bastion --> IGW
    
    style VPC fill:#f9f9f9,stroke:#333,stroke-width:2px
    style Public Subnet fill:#e1f5fe,stroke:#03a9f4,stroke-width:2px
    style Private Subnet fill:#ffebee,stroke:#00bcd4,stroke-width:2px